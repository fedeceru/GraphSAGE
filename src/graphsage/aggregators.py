...
class TwoMaxLayerPoolingAggregator(Layer):
    """
    Same idea as MaxPoolingAggregator, but the per-neighbor transform is a
    2-hidden-layer MLP instead of 1 (Section 3.3 notes the pooling MLP "can
    be an arbitrarily deep multi-layer perceptron, but we focus on simple
    single-layer architectures" -- this class is the 2-layer variant that was
    tried but not used for the main results). Not exposed as a ``--model``
    choice in the training scripts.
    """
    def __init__(self, input_dim, output_dim, model_size="small", neigh_input_dim=None,
            dropout=0., bias=False, act=tf.nn.relu, name=None, concat=False, **kwargs):
